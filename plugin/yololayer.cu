...
namespace nvinfer1
{
    ...
    __global__ void CalDetection(const float *input, float *output, int noElements,
        ...
        const float anchors[CHECK_COUNT * 2], int classes, int outputElem, bool is_segmentation)
    {
        int idx = threadIdx.x+blockDim.x*blockIdx.x;
        if (idx>noElements) return;
        int total_grid = yoloHeight*yoloWidth;
        ...
        {
            float box_prob=Logist(curInput[idx+total_grid*4 + k*info_len_i*total_grid]);
    ...
    void YoloLayerPlugin::forwardGpu(const float* const* inputs, float *output, cudaStream_t stream, int batchSize)
    {
        ...
        for (unsigned int i=0;i<mYoloKernel.size();i++)
        {
            ...
            numElem = yolo.width*yolo.width*batchSize;
